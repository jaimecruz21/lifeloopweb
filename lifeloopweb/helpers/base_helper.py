...
class Helper(object):
    ...
    def get_page_content(self, pagetype):
        with models.transaction() as session:
            query = session.query(models.Page)
            page = query.filter(
                models.Page.pagetype == pagetype).order_by(
                    models.Page.id.desc()).first()
        return page

    # pylint: disable=unexpected-keyword-arg,no-value-for-parameter
    def edit_page_content(self, form, email, pagetype):
        edit = models.Page(content=form.content.data,
                           title=form.title.data,
                           pagetype=pagetype,
                           updated_by=email)
        edit.save()

    def string_to_datetime_object(self, string):
        return datetime.datetime.strptime(string, '%m/%d/%Y %I:%M %p')

    def string_to_date_with_timezone(self, data):
        _datetime = self.string_to_datetime_object(data)
        string = self.datetime_offset(_datetime, current_user.timezone)
        final = self.string_to_datetime_object(string)
        return final
    ...
    def datetime_offset(dt, timezone=None):
        ...
        return pytz.utc.localize(
            dt, is_dst=None).astimezone(
                tz).strftime(constants.DATE_TIME_FORMAT)
    ...
    def text_message_form(self, data_id, form):
        textable = self.get_users(data_id, texting=True)
        choices = [("all", "Text All Users")]
        choices += [(u.id, u.full_name) for u in textable]
        form.recipient.choices = choices
        return form

    @staticmethod
    def need_leaders(org_id):
        flask.flash(
            'You must enter your billing information'
            ' before you may add a new group.', 'danger')
        return flask.redirect(flask.url_for('orgs.billing', org_id=org_id))
    ...
    def leader_alert_text(leader, sender, group, zoom_meeting):
        message_format_args = [
            ...
            zoom_meeting.topic]
        message = (
            '{}, this is a courtesy message from LifeLoop.Live to inform you'
            ' that {} {} has sent a mass reminder text to {} about their'
            ' meeting: {}.'.format(*message_format_args))
        return message
    ...
    def meeting_creator_alert_text(starter, group, zoom_meeting):
        ...
        message = (
            ...
            ' LifeLoop.Live meeting: {}. To start this meeting, please go'
            ' to the link below, or start the meeting from LifeLoop.Live at'
            ' the start time ({}). {}'.format(*message_format_args))
        return message
    ...
    def meeting_join_text(member, group, zoom_meeting):
        ...
        message_format_args.extend([zoom_meeting.info(current_user.timezone),
